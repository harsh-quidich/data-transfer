...
def handle_client(conn: socket.socket, out_dir: str, verbose: bool, expect_count_first: bool):
    try:
        with conn:
            ...
            while True:
                ...
                if verbose: print(f"[recv] <- {name} ({size} bytes)")
                ...
                conn.sendall(b'\x00')
                if verbose: print(f"[recv] ok  {name}")
...
def worker(bind: Tuple[str,int], out_dir: str, reuseport: bool, verbose: bool, expect_count_first: bool):
    ...
    srv.bind(bind)
    srv.listen(512)
    print(f"[recv] listening on {bind[0]}:{bind[1]} out_dir={out_dir} pid={os.getpid()}")
    try:
        while True:
            conn, _ = srv.accept()
            handle_client(conn, out_dir, verbose, expect_count_first)
...
def main():
    ap = argparse.ArgumentParser(description="Receiver v2 (diagnostics)")
    ...
    for _ in range(max(1, args.workers)):
        p = mp.Process(target=worker, args=((args.listen_ip, args.port), args.out_dir, args.reuseport, args.verbose, args.expect_count_first))
        p.start()
        procs.append(p)
    try:
